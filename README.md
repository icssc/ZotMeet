...
Scan with [Expo Go](https://expo.dev/go) (iOS: the Camera app; Android: Expo Go's scanner) to run the `main` branch on your phone. It talks to production (`zotmeet.com`). You must be signed in to Expo Go with an account in the ZotMeet organization; see [`apps/mobile/README.md`](apps/mobile/README.md#6-pr-previews-on-a-real-device-expo-go) for access and for the per-PR previews.
