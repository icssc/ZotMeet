...
<!--
  The QR code is generated by Expo from the query below and opens the latest
  EAS Update on the `main` channel, published by
  .github/workflows/mobile-main.yml on every push to main that touches the
  mobile bundle. `runtimeVersion` follows app.config.ts's `sdkVersion` policy
  (`exposdk:<Expo SDK major>.0.0`), so bump it here whenever apps/mobile
  upgrades the `expo` package, then run the workflow by hand once.
-->
<a href="https://expo.dev/accounts/ethanchaos-team/projects/zotmeet/updates">
  <img
    src="https://qr.expo.dev/eas-update?appScheme=exp&projectId=7280fa26-1305-4465-b2a7-8b6b8665b3e6&channel=main&runtimeVersion=exposdk%3A57.0.0"
    alt="QR code: open the main branch of the ZotMeet mobile app in Expo Go"
    width="200"
  />
</a>

Scan with [Expo Go](https://expo.dev/go) (iOS: the Camera app; Android: Expo Go's scanner) to run the `main` branch on your phone — no Xcode, no local checkout. It talks to production (`zotmeet.com`). You need to be signed in to Expo Go with an account in the **`ethanchaos-team`** Expo organization; see [`apps/mobile/README.md`](apps/mobile/README.md#6-pr-previews-on-a-real-device-expo-go) for access and for the per-PR previews.
...
3. Set `PWA_IARC_RATING_ID` after completing the free IARC questionnaire through
   a participating storefront (see [How IARC Works](https://www.globalratings.com/how-iarc-works/)).
   Update `NEXT_PUBLIC_IOS_BUNDLE_ID` if PWA Builder assigns a different bundle ID.
...
Regenerates icons under `public/icons/` and store screenshots under
`public/screenshots/`.
