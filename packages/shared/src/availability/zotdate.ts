/**
 * One calendar day of an availability grid. Lives in `@zotmeet/shared` so the
 * web and Expo apps compute cells from the same model; the web re-exports it
...
export class ZotDate {
	...
	/**
	 * Converts block index to ISO string. This is the one encoding of a grid
	 * slot — `getTimestampFromBlockIndex` in `./grid` delegates here.
	 * @param blockIndex the index of the time block
	 * @return ISO string representing the start time of the block
	 */
	getISOStringForBlock(blockIndex: number): string {
