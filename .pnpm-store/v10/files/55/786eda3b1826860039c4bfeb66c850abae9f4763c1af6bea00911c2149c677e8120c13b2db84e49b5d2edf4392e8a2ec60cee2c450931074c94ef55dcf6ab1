...
async function migrate(db, config) {
	...
	if (typeof config === "object" && config.init) {
		...
		if (!migration) return;
		await db.run(__sql_sql_ts.sql`INSERT INTO ${__sql_sql_ts.sql.identifier(migrationsTable)} ("hash", "created_at") VALUES(${migration.hash}, '${migration.folderMillis}')`.inlineParams());
		return;
	}
	...
	for (const migration of migrations) if (!lastDbMigration || Number(lastDbMigration[2]) < migration.folderMillis) {
		...
		statementToBatch.push(db.run(__sql_sql_ts.sql`INSERT INTO ${__sql_sql_ts.sql.identifier(migrationsTable)} ("hash", "created_at") VALUES(${migration.hash}, '${migration.folderMillis}')`.inlineParams()));
	}
