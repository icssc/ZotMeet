...
async function migrate(db, config) {
	...
	if (typeof config === "object" && config.init) {
		...
		if (!migration) return;
		await db.session.execute(sql`insert into ${sql.identifier(migrationsSchema)}.${sql.identifier(migrationsTable)} ("hash", "created_at") values(${migration.hash}, ${migration.folderMillis})`);
		return;
	}
