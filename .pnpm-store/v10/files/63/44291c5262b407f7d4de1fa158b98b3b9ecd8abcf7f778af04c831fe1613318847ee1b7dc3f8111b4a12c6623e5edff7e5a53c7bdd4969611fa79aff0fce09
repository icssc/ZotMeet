...
var PrimaryKeyBuilder = class {
	static [__entity_ts.entityKind] = "SQLitePrimaryKeyBuilder";
	/** @internal */
	columns;
	/** @internal */
	name;
...
var PrimaryKey = class {
	static [__entity_ts.entityKind] = "SQLitePrimaryKey";
	columns;
	name;
	isNameExplicit;
	...
	getName() {
		return this.name ?? `${this.table[require_sqlite_core_table.SQLiteTable.Symbol.Name]}_${this.columns.map((column) => column.name).join("_")}_pk`;
	}
};
