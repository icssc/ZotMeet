...
var require_dist_cjs$2 = /* @__PURE__ */ require_chunk.__commonJSMin(((exports) => {
	...
	const getSsoOidcClient = async (ssoRegion, init = {}) => {
		const { SSOOIDCClient } = await Promise.resolve().then(() => require("./sso-oidc-C---moSr.js"));
	...
	const getNewSsoOidcToken = async (ssoToken, ssoRegion, init = {}) => {
		const { CreateTokenCommand } = await Promise.resolve().then(() => require("./sso-oidc-C---moSr.js"));
