...
needsManifestsForLegacyReasons }) {
    ...
    if (needsManifestsForLegacyReasons) {
        ...
        if (!process.env.TURBOPACK) {
            ...
            reactLoadableManifestPath = join(/* turbopackIgnore: true */ distDir, 'server', 'app', page, REACT_LOADABLE_MANIFEST);
        } else {
            reactLoadableManifestPath = join(/* turbopackIgnore: true */ distDir, 'server', 'pages', normalizePagePath(page), REACT_LOADABLE_MANIFEST);
        }
        ...
        const [buildManifest, reactLoadableManifest, dynamicCssManifest, clientReferenceManifest, serverActionsManifest, subresourceIntegrityManifest] = await Promise.all([
            loadManifestWithRetries(join(/* turbopackIgnore: true */ distDir, BUILD_MANIFEST), manifestLoadAttempts),
            ...
            sriEnabled ? loadManifestWithRetries(join(/* turbopackIgnore: true */ distDir, 'server', SUBRESOURCE_INTEGRITY_MANIFEST + '.json')).catch(()=>undefined) : undefined
        ]);
